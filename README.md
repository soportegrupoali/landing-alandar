...
- `src/content/site.yaml` — **todo el copy editable** (textos, stats, financiamiento, config del video). Es lo que edita el panel `/admin`; editarlo a mano también es válido.
...
- `public/admin/` — panel de contenido (Decap CMS, ver abajo).
- `cms-auth-worker/` — Worker aparte que atiende el login del panel (OAuth de GitHub).

## Panel de contenido (CMS)

`/admin` — edición de copy sin tocar código, ver **`SETUP-CMS.md`** para los
pasos de configuración pendientes (crear la GitHub OAuth App) y cómo
funciona el acceso.
...
2. **Teléfono/WhatsApp y correo de ventas reales**: en `src/content.yaml` → `contact.phoneDisplay`, `contact.phoneWhatsapp`, `contact.email`. Hoy tienen valores de ejemplo. El botón flotante de WhatsApp y los formularios (contacto, brochure, info) dependen de `phoneWhatsapp`.
...
4. **CMS**: falta crear la GitHub OAuth App y configurar el Client ID/Secret — ver `SETUP-CMS.md`.
...
- Sitio: https://landing-alandar-2026.landing-alandar-2026.workers.dev
- El video local (`alandar-04.mp4`, 27 MiB) queda excluido del deploy vía `public/.assetsignore` — Cloudflare Workers Assets tiene un límite de 25 MiB por archivo. Para producción real, conviene usar Vimeo (ya soportado, ver `SETUP-CMS.md`) o subir el mp4 a otro storage.
